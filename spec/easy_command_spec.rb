...
describe EasyCommand do
  ...
  describe "#call" do
    ...
    it "raises an exception if the method is not defined in the command" do
      ...
      end.to raise_error(EasyCommand::NotImplementedError)
    end
  ...
  describe '#abort' do
    ...
    it "should let Failure carry the result" do
  ...
  describe "#on_success" do
    ...
    it "is executed after #call" do
      command.call
      expect(command).to have_received(:call).ordered
      expect(command).to have_received(:on_success).ordered
    end
    ...
    context "when there are errors" do
      ...
      it "does not call #on_success" do
        command.call
        expect(command).to have_received(:call).ordered
        expect(command).not_to have_received(:on_success)
      end
    end
    ...
    context "when using sub command" do
      ...
      specify "#on_success are called in order" do
        command.call
        expect(command).to have_received(:on_success).ordered
        expect(subcommand).to have_received(:on_success).ordered
      end

      specify "Sub command #on_success code are executed before parent" do
        command.call
        expect(subcommand).to have_received(:code_execution).ordered
        expect(command).to have_received(:code_execution).ordered
      end
      ...
      context "when parent has errors" do
        ...
        it "does not call #on_success neither for parent nor for children" do
          command.call
          expect(command).not_to have_received(:on_success)
          expect(subcommand).not_to have_received(:on_success)
        end
      end
